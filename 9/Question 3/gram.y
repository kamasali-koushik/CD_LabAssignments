...
Node* createNode(string opr,Node* temproot1, Node* temproot2)
{
	...
	{           
		...
		string left;
		if(temproot1->isop)
		left = "t_"+to_string(temproot1->t_id);
		else
		left = to_string(temproot1->val);

		string right;
		if(temproot2->isop)
		right = "t_"+to_string(temproot2->t_id);
		else
		right = to_string(temproot2->val);

		string address = "t_"+to_string(root->t_id)+"="+left+opr+right;
